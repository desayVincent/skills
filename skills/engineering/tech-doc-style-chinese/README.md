...
内部技术文档以问题、证据、解释和实现影响组织，随后修复拗口表达并检查模板句式。入口仍为 SKILL.md；API、UI、操作、校对和只读审阅保持各自范围。

- [技术主流程](references/technical-polish.md)：从读者问题到论证、实现影响和旧文更新。
- [表达修复](references/plain-language.md)：按症状选择方法，提供前后例与保留边界。
- [模式检查](references/ai-patterns.md)：人工检查线索、修复与例外，不输出作者概率。
- [来源及取舍](references/upstream-sources.md)：固定上游提交、适配覆盖和验证局限。
- [本地语义验证](tests/README.md)：样例、判定和本次结果；不是效果保证。

规则和许可声明随本目录分发，使用时不联网下载或调用上游 skill。维护者需要更新上游时才获取新版本并复核差异。
