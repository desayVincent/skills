...
**Done when:** every operation introduced or moved by the change is legal in
all contexts that can reach it.
...
by one named mechanism, including teardown.
...
re-entrancy, and latency justification.
...
and release path, with no user remaining after lifetime ends.

## Completion checklist

- [ ] Every touched entry point has a CTX and sleepability decision.
- [ ] Every shared object has named ownership, synchronization, and ordering.
- [ ] Every held-lock call has legality, lock-order, re-entrancy, and latency evidence.
