...
| HOT | 热路径档：禁止仅为行数强拆；允许单 leaf 偏长与调用预算 0；仍约束嵌套、副作用与共享数据纪律。用于 ISR 与极紧 fast path。 |
...
| HOT Rules (H1–H6) | ISR∩HOT 的硬规则集：H1 关闭 15/40 行数目标；H2 调用预算默认 0（例外须注释）；H3 能延后必须延后；H4 跨上下文共享数据须标明同步手段；H5 禁止阻塞/睡眠/无界等待/不当分配；H6 入口须标注 PATH/CTX。 |
| Overlay Doc Layers | Overlay 文档切分：L0 正文（两轴、优先级、H1–H6、BOUND/ORCH/DRIVER 与仓库类型通则，每次必读）；L1 平台附录（Linux/RT-Thread/Zephyr 仅冲突对照表，按需加载）；L2 不进 skill（完整内核/API 手册，查官方文档）。 |
...
| Hard Order | Agent 固定工作流：① Active Git Root + Keyword Heuristic → ② Repo Kind / Path Class / CTX → ③ 加载 Bundle 内 Base + Overlay L0 → ④ 内核则 L1 附录 → ⑤ 改代码 → ⑥ checklist（HOT 则含 H1–H6）。 |
