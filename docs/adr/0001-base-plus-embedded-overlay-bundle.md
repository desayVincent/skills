...
2. Vendor Base **write-legible-c** (MIT, 7etsuo) under `references/base/`; HOST branch runs Base procedure in-process (no second skill install). Rule **semantics** stay upstream; packaging may progressive-disclose examples/provenance (former §§15–18) into sibling files so the agent default-loads §§1–14 only.
...
- Progressive disclosure of Base examples (§§15–16) and provenance (§§17–18) into sibling files under `references/base/` reduces default context load; MIT attribution still covers all Base files.
